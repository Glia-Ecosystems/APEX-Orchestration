...
public class ResourceHandler {
    ...
    public ResourceHandler(final ApplicationContext applicationContext,
                           final ObjectMapper objectMapper, final ResourcesLoader resourcesLoader) {
    ...
    public void init() {
        ...
        for (final Class<?> clazz : resourcesLoader.getClasses()) {
            final RequestMapping uri = clazz.getAnnotation(RequestMapping.class);
            ...
            resourceMap.put(Arrays.toString(uri.value()) + "(/.*)?", resource);
        }
    }
    ...
    public ResponsePayload processRequest(RequestPayload request) {
        ...
        final String methodUri = request.getResourceURI().replace(Arrays.toString(requestedResource.getPathURI().value()), "");
    ...
    private Object[] getMethodArguments(final RequestPayload requestPayload, final ResourceMethod resourceMethod,
        ...
        for (int i = 0; i < parameters.size(); i++) {
            ...
            if (parameterAnnotationType.equals("PATH")) {
                arguments[i] = getPathParamValue(requestPayload.getResourceURI(), Arrays.toString(resourceMethod.getUri().value()),
                        parameters.get(i).getParameterName(), Arrays.toString(requestedResource.getPathURI().value()));
